...
#define DEVICE_NAME                      "BM+S42_Template-Max"                          /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "Telit Wireless Solutions"                 /**< Manufacturer. Will be passed to Device Information Service. */
...
#define BATTERY_LEVEL_MEAS_INTERVAL      APP_TIMER_TICKS(8000, APP_TIMER_PRESCALER) /**< Battery level measurement interval (ticks). */
...
static void gap_params_init(void)
{
    ...
    /* Telit specific functionality start:                                            */		
    /* set public Bluetooth Address from UICR if available ( programmed in testmode ) */
       ...
       p_UICR[3] != 0xFF || p_UICR[4] != 0xFF || p_UICR[5] != 0xFF )			
    {
      ...
      addr.addr[0] = p_UICR[5];
      addr.addr[1] = p_UICR[4];
      addr.addr[2] = p_UICR[3];
      addr.addr[3] = p_UICR[2];
      addr.addr[4] = p_UICR[1];
      addr.addr[5] = p_UICR[0];
      err_code = sd_ble_gap_address_set(BLE_GAP_ADDR_CYCLE_MODE_NONE, &addr) 	;
      APP_ERROR_CHECK(err_code);
    }
    /* Telit specific functionality end */		
