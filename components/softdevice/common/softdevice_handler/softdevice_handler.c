...
{
    uint32_t err_code;
    //SEGGER_RTT_printf(0,"Softdevice Handler Init\n");																					\
    SD_HANDLER_LOG_INIT();
    //SEGGER_RTT_printf(0,"Softdevice Handler Log Init Complete\n");																					\

    // Save configuration.
#if defined (BLE_STACK_SUPPORT_REQD)
    // Check that buffer is not NULL.
		//SEGGER_RTT_printf(0,"BLE_STACK_SUPPORT_REQD\n");
    if (p_ble_evt_buffer == NULL)
    {
        //SEGGER_RTT_printf(0,"BLE Event Buffer is null\n");
    ...
    {
        //SEGGER_RTT_printf(0,"Buffer is not correctly aligned\n");
				return NRF_ERROR_INVALID_PARAM;
    }
...
#else
		//SEGGER_RTT_printf(0,"Enabling Softdevice\n");
    err_code = sd_softdevice_enable(p_clock_lf_cfg, softdevice_fault_handler);
#endif
    if (err_code != NRF_SUCCESS)
    {
        //SEGGER_RTT_printf(0,"Not Successful\n");
				return err_code;
    }
    ...
    // Enable BLE event interrupt (interrupt priority has already been set by the stack).
		...
		//SEGGER_RTT_printf(0,"SOFTDEVICE PRESENT\n");
    ...
    //SEGGER_RTT_printf(0,"RETURNING SUCCESS\n");
		return NRF_SUCCESS;
#endif
}
