 ...
 * Furthermore, this code also implements ADC sampling. An additional application timer is
 * set up to trigger the ADC sampling which is performed 10 times per second. ADC is sampled
 * with 8 bit resolution, samples from analog input pin 2, uses the internal 1.2VBG fixed
...
#define DEVICE_NAME                          "Nordc_HRM"                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                    "NordicSemiconductor"                      /**< Manufacturer. Will be passed to Device Information Service. */
#define APP_ADV_INTERVAL                     3500                                         /**< The advertising interval (in units of 0.625 ms. This value corresponds to 25 ms). */
...
static void leds_init(void)
{
    ...
    GPIO_LED_CONFIG(CONNECTED_LED_PIN_NO);
		GPIO_LED_CONFIG(NRF6310_LED_2);
	  GPIO_LED_CONFIG(NRF6310_LED_3);
...
void ADC_IRQHandler(void)
{
  ...
  /* Write ADC result to port 2 */
	...
	nrf_gpio_pin_toggle(NRF6310_LED_3);
