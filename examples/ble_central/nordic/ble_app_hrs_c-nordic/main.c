                                                 ...
                                                 const ret_code_t     event_result)
{
    ...
    switch (p_event->event_id)
    {
        ...
        {
						SEGGER_RTT_printf(0,"DM CONNECTION EVENT\n");
...
{
		unsigned char HPEHID[6] = {0x8E, 0xBB, 0xFA, 0x55, 0x1C, 0xF4};
		bool IDMatch = true;
    uint32_t                err_code;
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        {
            //SEGGER_RTT_printf(0,"Advertisement Packet Received\n");
						//data_t adv_data;
            //data_t type_data;
						for (int i = 0;i<6;i++)
						{
								//SEGGER_RTT_printf(0,"Adv Addr is %x\n",p_gap_evt->params.adv_report.peer_addr.addr[i]);
								//SEGGER_RTT_printf(0,"HPEH Addr is %x\n",HPEHID[i]);
								if (!((p_gap_evt->params.adv_report.peer_addr.addr[i]) == HPEHID[i]))
								{
									IDMatch = false;
								}
            }
						if (!IDMatch) {
							//SEGGER_RTT_printf(0, "Peer Address does not match\n");
							break;
						}
						SEGGER_RTT_printf(0,"\n");						
            ...
            SEGGER_RTT_printf(0,"Initiating Connection\n");
...
/*
        ...
        {
						SEGGER_RTT_printf(0,"BLE_GAP_EVT_CONNECTED Event Received\n");
...
{
    if (connected) {
		SEGGER_RTT_printf(0,"Event Dispatch\n");
		}
    ...
    bsp_btn_ble_on_ble_evt(p_ble_evt);
		if (connected) {
		SEGGER_RTT_printf(0,"BSP_BTN_BLE_ON_BLE_EVT Completed\n");
		}
    on_ble_evt(p_ble_evt);
		if (connected) {
		SEGGER_RTT_printf(0,"On_BLE_Evt Completed\n");
		}
}
...
{
    SEGGER_RTT_printf(0,"BLE Stack Init Starting\n");

		uint32_t err_code;
		
    nrf_clock_lf_cfg_t clock_lf_cfg = NRF_CLOCK_LFCLKSRC;
		//nrf_clock_lf_cfg_t clock_lf_cfg = NRF_CLOCK_LFCLKSRC_RC_250_PPM_8000MS_CALIBRATION;
	
    // Initialize the SoftDevice handler module.
    SOFTDEVICE_HANDLER_INIT(&clock_lf_cfg, NULL);
    //SEGGER_RTT_printf(0,"SoftDevice Handler Init Complete\n");
    ...
    err_code = softdevice_enable_get_default_config(CENTRAL_LINK_COUNT,
		...
		APP_ERROR_CHECK(err_code);
		//SEGGER_RTT_printf(0,"Get Default Config Error Check Complete\n");

    //Check the ram settings against the used number of links
    CHECK_RAM_START_ADDR(CENTRAL_LINK_COUNT,PERIPHERAL_LINK_COUNT);
    //SEGGER_RTT_printf(0,"Check RAM Start Complete\n");

    // Enable BLE stack.
    
		err_code = softdevice_enable(&ble_enable_params);
    APP_ERROR_CHECK(err_code);
		//SEGGER_RTT_printf(0,"SoftDevice Enable Error Check Complete\n");
...
int main(void)
{
    ...
    SEGGER_RTT_printf(0,"\n\nCentral Program\n");
    ble_stack_init();
    SEGGER_RTT_printf(0,"BLE Stack Init Complete\n");
