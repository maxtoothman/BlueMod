 ...
 */
static uint8_t hrm_encode(ble_hrs_t * p_hrs, uint64_t heart_rate, uint8_t * p_encoded_buffer)
{
    uint8_t flags = 0;
    uint8_t len   = 1;
    int     i;
    ...
    if (heart_rate > 0xff)
    {
        ...
        len   += uint64_encode(heart_rate, &p_encoded_buffer[len]);
    }
    else
    {
...
uint32_t ble_hrs_heart_rate_measurement_send(ble_hrs_t * p_hrs, uint64_t heart_rate)
{
    uint32_t err_code;
