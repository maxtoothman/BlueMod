...
#define MIN_CONN_INTERVAL                MSEC_TO_UNITS(8, UNIT_1_25_MS)           /**< Minimum acceptable connection interval (0.4 seconds). */
#define MAX_CONN_INTERVAL                MSEC_TO_UNITS(8, UNIT_1_25_MS)           /**< Maximum acceptable connection interval (0.65 second). */
...
//    heart_rate = (uint16_t)sensorsim_measure(&m_heart_rate_sim_state, &m_heart_rate_sim_cfg);
		//uint64_t heart_rate = 18446744073709551615;
	
    heart_rate = (((uint64_t)((uint8_t *)samples)[0]) << 0)|
							 (((uint64_t)((uint8_t *)samples)[1]) << 8)  |
               (((uint64_t)((uint8_t *)samples)[2]) << 16) |
               (((uint64_t)((uint8_t *)samples)[3]) << 24) |
               (((uint64_t)((uint8_t *)samples)[4]) << 32) |
               (((uint64_t)((uint8_t *)samples)[5]) << 40) |
						   (((uint64_t)((uint8_t *)samples)[6]) << 48) |
               (((uint64_t)((uint8_t *)samples)[7]) << 56);
		...
		//heart_rate = adcSampleCount;
	
		//heart_rate = ((uint64_t)((uint8_t *)samples)[0]);
...
void rtc_config(void)
{
  ...
  nrf_drv_rtc_enable(&rtc);
	...
	uint32_t rtc_tick_event_addr = nrf_drv_rtc_event_address_get(&rtc, NRF_RTC_EVENT_TICK);
	uint32_t saadc_sample_event_addr = nrf_drv_saadc_sample_task_get();

  // setup ppi channel so that timer compare event is triggering sample task in SAADC 
  /*err_code = nrf_drv_ppi_channel_alloc(&m_ppi_channel);
  APP_ERROR_CHECK(err_code);  
  err_code = nrf_drv_ppi_channel_assign(m_ppi_channel, rtc_tick_event_addr, saadc_sample_event_addr);
  APP_ERROR_CHECK(err_code);
  ...
  if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
    {
	  ...
	  adcSampleCount=adcSampleCount + 1;
		}
