...
#define OUR_SERVICE_UUID_TYPE   BLE_UUID_TYPE_VENDOR_BEGIN      /**< UUID type for the Nordic UART Service (vendor specific). */
...
static ble_our_service_c_t      m_ble_our_service_c;                    /**< Instance of NUS service. Must be passed to all NUS_C API calls. */
...
static const ble_uuid_t m_our_service_uuid = 
  {
    .uuid = BLE_UUID_OUR_SERVICE_UUID,
    .type = OUR_SERVICE_UUID_TYPE
  };
...
static void ble_our_service_c_evt_handler(ble_our_service_c_t * p_ble_our_service_c, const ble_our_service_c_evt_t * p_ble_our_service_evt)
{
    uint32_t err_code;
    switch (p_ble_our_service_evt->evt_type)
    {
        case BLE_OUR_SERVICE_C_EVT_DISCOVERY_COMPLETE:
            err_code = ble_our_service_c_handles_assign(p_ble_our_service_c, p_ble_our_service_evt->conn_handle, &p_ble_our_service_evt->handles);
            APP_ERROR_CHECK(err_code);

            err_code = ble_our_service_c_our_service_notif_enable(p_ble_our_service_c);
            APP_ERROR_CHECK(err_code);
            APPL_LOG("The device has the Nordic UART Service\r\n");
            break;
        /*
        case BLE_OUR_SERVICE_C_EVT_OUR_SERVICE_RX_EVT:
            for (uint32_t i = 0; i < p_ble_our_service_evt->data_len; i++)
            {
                while(app_uart_put( p_ble_our_service_evt->p_data[i]) != NRF_SUCCESS);
            }
            break;
        */
        case BLE_OUR_SERVICE_C_EVT_DISCONNECTED:
...
void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
        ...
        case BSP_EVENT_DISCONNECT:
            err_code = sd_ble_gap_disconnect(m_ble_our_service_c.conn_handle,
...
{
    uint32_t         err_code;
    ble_our_service_c_init_t our_service_c_init_t;
    
    our_service_c_init_t.evt_handler = ble_our_service_c_evt_handler;
    
    err_code = ble_our_service_c_init(&m_ble_our_service_c, &our_service_c_init_t);
    APP_ERROR_CHECK(err_code);
}
